...
export default (async () => {
    ...
    new synced.S3BucketFolder(`${resourcePrefix}-synced-folder`, {
        path: '../dist/tpf-character-sheet/browser',
        bucketName: bucket.bucket,
        acl: 'private',
    })

    new aws.s3.BucketOwnershipControls(`${resourcePrefix}-bucket-ownership-controls`, {
        bucket: bucket.bucket,
        // For options check https://docs.aws.amazon.com/AmazonS3/latest/userguide/about-object-ownership.html
        rule: { objectOwnership: 'BucketOwnerPreferred' },
    })

    const originAccessControl = new aws.cloudfront.OriginAccessControl(`${resourcePrefix}-origin-access-control`, {
    ...
    const cloudfrontDistribution = new aws.cloudfront.Distribution(`${resourcePrefix}-cloudfront-distribution`, {
        origins: [
            {
